...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('acdleaderboard')
		.setDescription('Replies with auto-updating ACD leaderboard'),
	async execute(message) {
		...
		const botpanelembed = new EmbedBuilder()
            .setColor(`#5865F2`)
            .setTitle(`ACD Leaderboard: <t:${Math.floor(Date.now()/1000)}:d> at <t:${Math.floor(Date.now()/1000)}:T>`)
        var userNames = ''
        var userHours = ''
